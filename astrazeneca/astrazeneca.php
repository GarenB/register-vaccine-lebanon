...
<div class="back-pic">
  <div class="admin">
  <h3  class="profile-left" >Astrazeneca</h3>
  </div>
</div>
...
<div class="back-pic3">
<div>
 <h5 class="astra-intro">AstraZeneca plc is a British-Swedish multinational pharmaceutical and biotechnology company with its headquarters at the Cambridge Biomedical Campus in Cambridge, England.</h5>
 </div>   
<h4 class="data-title2">Charts: % of people from each city that registered to take Astrazeneca vaccine</h4>
...
<?php 
    ...
    $sql = "SELECT * FROM register, users, placedatetime, locations WHERE users.LocationID=locations.locationID AND register.patientUsername = users.Username AND register.pdtID=placedatetime.ID AND placedatetime.vaccineID='2' ";
                  $result = mysqli_query($db,$sql);             
                  if (mysqli_num_rows($result) > 0) {
